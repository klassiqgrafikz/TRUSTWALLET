...
function generateChainAddress(chainKey,walletAddr){
  const seed=hashStr(walletAddr+chainKey);
  const net=NETWORKS[chainKey];if(!net)return walletAddr;
  switch(chainKey){
    ...
    default:return walletAddr;
  }
}
function initChainAddresses(){
  if(!state.wallet)return;
  const addr=state.wallet.address;
  Object.keys(NETWORKS).forEach(k=>{state.chainAddresses[k]=generateChainAddress(k,addr)});
}
function getChainAddress(chainKey){return state.chainAddresses[chainKey]||state.wallet.address}
